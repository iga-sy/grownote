...
- 上流思考(考える): 情報収集・分析(背景・前提・事実を収集/見立て整理)/課題把握(目的・範囲・影響を置く/論点化)/方針設計・仮説検証(仮説・選択肢・比較観点を置く)
- 遂行(やりきる): 表現(相手が判断できる形で伝える)/課題解決(たたき台を作り、確認・修正しながら進める)/リレーション構築(巻き込み/調整)
- 対人・管理(崩れない・再現する): リスク管理(品質/期限/影響範囲のリスク確認)/指導・共有(形式知化・引き継げる形で残す)/リソースマネジメント(作業量/期限/優先順位)`;
...
# 出力形式(Markdown、この見出し構成のみを使用。育成面談シートの週次欄に沿った構成)
## 今週の計画・意識すること
## 振り返り
### ①やったこと
### ②分からなかった・確認したこと
...
- 「①やったこと②分からなかった・確認したこと③来週試すこと」は必ずこの順番で記入してください。
...
# 出力形式(Markdown、この見出し構成のみを使用。育成面談シートの月次欄に沿った構成)
## ①今月の目標
## ②今月の目標達成度合い
### ②-1 結果(何をした・何が起きたか/数値・事実)
### ②-2 技量の観点での振り返り
## ③来月に向けての改善案
## ④成果物・提出物一覧

- 「①今月の目標」は、上記「今月の目標」をそのまま簡潔に転記してください。目標が設定されていない場合は「(今月の目標は設定されていません)」と書いてください。
- 「②-1 結果」は、週報を単に並べるのではなく、月を通した傾向・共通の課題・重要な出来事を要約し、数値や事実を根拠に書いてください。
- 「②-2 技量の観点での振り返り」では、上記の技量の観点のうち、根拠となるエピソードがあるものだけを取り上げ、「意識できた場面」と「上手くいかなかった点」の両方に触れてください。
- 「③来月に向けての改善案」は②を踏まえた具体的な次のアクションにしてください。
- 「④成果物・提出物一覧」には、上記「この月に提出・完了した成果物」を1件も省略せず、まとめたり間引いたりせず、箇条書きで逐一列挙してください。ファイルURLがあれば併記してください。
- 記録が少ない場合は、無理に水増しせず簡潔にまとめてください。
- 「だ・である」調で、できる限り箇条書き中心に書いてください。`;
}
...
interface GoalForReview {
  title: string;
  progress: number;
  parentTitle?: string | null;
}

interface ReportForGoalReview {
  label: string;
  content: string;
}

export function buildGoalReviewPrompt(
  scope: "weekly" | "monthly",
  periodLabel: string,
  goals: GoalForReview[],
  sourceReports: ReportForGoalReview[],
): string {
  const scopeLabel = scope === "weekly" ? "週間目標" : "月間目標";
  const sourceLabel = scope === "weekly" ? "日報" : "週報";

  const goalLines = goals.length
    ? goals
        .map(
          (g) =>
            `- ${g.title}(進捗${g.progress}%${g.parentTitle ? `、紐づく月間目標:${g.parentTitle}` : ""})`,
        )
        .join("\n")
    : `(${scopeLabel}は設定されていません)`;

  const reportLines = sourceReports.length
    ? sourceReports.map((r) => `### ${r.label}\n${r.content}`).join("\n\n")
    : `(この期間の${sourceLabel}の記録なし)`;

  return `あなたは新入社員の目標達成を支援するコーチです。
対象期間: ${periodLabel}

以下の「${scopeLabel}」と、この期間に書かれた「${sourceLabel}」を照らし合わせ、目標に対する振り返りを日本語で作成してください。
...
# 出力形式(Markdown、この見出し構成のみを使用)
## 目標に対する進捗評価
## ${sourceLabel}から見える不足・懸念点
## 次に取り組むべきこと

- 「目標に対する進捗評価」は、自己申告の進捗率だけでなく、${sourceLabel}に実際に書かれている内容を根拠に評価してください。
- 「${sourceLabel}から見える不足・懸念点」は、目標に対して具体的に何が足りていないか、取り組みが見えない目標は何かを名指しで指摘してください。曖昧な励ましで終わらせないでください。
- 「次に取り組むべきこと」は、指摘した不足点を踏まえた具体的なアクションにしてください。
- ${scopeLabel}が1件も設定されていない場合は、その旨を伝え、まず目標を設定するよう促してください。
- ${sourceLabel}の記録が少ない場合は、無理に指摘を捻り出さず、記録が少ないこと自体を懸念点として指摘してください。
- 「だ・である」調で、できる限り箇条書き中心に書いてください。`;
}

export async function generateGoalReview(prompt: string): Promise<string> {
  return callGemini(prompt);
}
